...
part1(std::istream& _is)
{
    ...
    auto code = get_code(
        data, [](tDataDataIt first, tDataDataIt last) { return std::max_element(first, last); });
...
part2(std::istream& _is)
{
    auto data = get_data(_is);
    auto code = get_code(
        data, [](tDataDataIt first, tDataDataIt last) { return std::min_element(first, last); });
...
template <>
std::string
solve<kDay06>(bool _part1, std::istream& _is, [[maybe_unused]] std::ostream& _os)
{
