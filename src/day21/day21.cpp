...
std::regex const swap_pos_re{R"(swap position (\d+) with position (\d+))", std::regex_constants::optimize};
...
std::regex const rotate_pos_re{R"(^rotate based on position of letter (.)$)", std::regex_constants::optimize};
std::regex const reverse_pos_re{R"(^reverse positions (\d+) through (\d+)$)", std::regex_constants::optimize};
std::regex const move_pos_re{R"(^move position (\d+) to position (\d+)$)", std::regex_constants::optimize};
...
scramble(std::vector<std::string> const& instructions, std::string str)
{
	...
	for (auto it = instructions.cbegin(); it != instructions.cend(); ++it) {
		...
		if (std::regex_match(instruction, matches, swap_pos_re)) {
			...
			auto dir = (matches[1] == "left") ? Scrambler::Direction::LEFT : Scrambler::Direction::RIGHT;
