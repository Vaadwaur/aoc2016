...
part1(tNodes const& nodes)
{
    ...
    for (auto const& node : nodes) {
        for (auto const& i_node : nodes) {
            if (node.first != i_node.first && node.second.used > 0 &&
                node.second.used <= i_node.second.avail) {
                ++counter;
            }
        }
    }
...
part2(tNodes const& nodes, std::ostream& _os)
{
    ...
    _os << "Empty node (_) at pos " << empty_pos_x << ',' << empty_pos_y
        << "\n1. Taxicab geometry - move _ to the top row. (diff x + diff y moves). # cannot be "
           "crossed.\n"
